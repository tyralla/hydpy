...
class Q9(sequencetools.LogSequence):
...
class Q1(sequencetools.LogSequence):
    ...
    >>> x4(3.4)
    >>> derived.uh2.update()
    >>> logs.q1(1.0, 2.0, 1.0, 3.0, 4.0, 6.0, 0.0)
    >>> logs.q1
    q1(1.0, 2.0, 1.0, 3.0, 4.0, 6.0, 0.0)
    ...
    Traceback (most recent call last):
    ...
    UserWarning: Due to the following problem, log sequence `q1` of \
element `?` handling model `grxjland` could be initialised with a averaged \
value only: While trying to set the value(s) of variable `q1`, the \
following error occurred: While trying to convert the value(s) \
